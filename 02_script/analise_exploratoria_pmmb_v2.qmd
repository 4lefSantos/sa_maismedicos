...
Assim, iniciaremos caracterizando as variáveis e examinando sua distribuição por meio de gráficos como boxplots, histogramas e identificação de outliers. Em seguida, realizaremos uma análise desagregada considerando o churn, ou seja, distinguindo entre médicos que Permaneceuram ou migraram, respeitando a estrututura de atributos relacionados ao indivíduo, estabelecimento e município. E por fim, o diagrama de Sankey para entender como os profissionais transitaram no território brasileiro por região no período de 4,5 anos após o início do programa.
...
```{r}
data <- read_xlsx("~/GitHub/sa_maismedicos/01_dados/Editais/2019/database.xlsx")
...
+------------------------------------------+--------------------------------------------------------------------------------------------------------------------+
| churn                                    | Variável binária que identifica os médicos que Permaneceuram no município em que foi destinado após 4,5 anos       |
+------------------------------------------+--------------------------------------------------------------------------------------------------------------------+
...
Antes de iniciar a análise exploratória dos dados, definiremos duas funções que serão úteis e que diminuirá a extensão do código. A primeira para variáveis quantitativas, com o cálculo dos quartis e limites inferior e superior, e a segunda para variáveis qualitativas para cálculo de proporção das categorias.

```{r}

estatistica_descritiva <- function(variavel) {
  data |> 
    summarise(
      Mínimo = min({{variavel}}, na.rm = TRUE),
      Q1 = quantile({{variavel}}, 0.25, na.rm = TRUE),
      Mediana = median({{variavel}}, na.rm = TRUE),
      Q3 = quantile({{variavel}}, 0.75, na.rm = TRUE),
      Máximo = max({{variavel}}, na.rm = TRUE)
    ) |> 
    mutate(
      IQR = Q3 - Q1,
...
estatistica_qualitativa <- function(variavel){
  data |> 
  group_by({{variavel}}) |> 
  summarise(total = n()) |> 
...
sexo <- estatistica_qualitativa(sexo)
...
atuacao_previa <- estatistica_qualitativa(atuacao_previa_no_municipio)
...
regiao_destino <- estatistica_qualitativa(regiao_destino)
...
uf_final |> 
  ...
  geom_text(aes(label = paste0(total," (",percentual, "%)")), hjust = -0.2) + 
  labs(title = "Estado de atuação após 4,5 anos",
       x = NULL, 
       y = NULL) + 
  theme_minimal()
```
...
```{r}
renovacao <- estatistica_qualitativa(Prorrogado)
...
A variável churn como descrito anteriormente, indica quantos profissionais Permaneceuram nas cidades em que foram destinados e quantos migraram após um período de 4,5 anos. Observa-se também um balanceamento por churn, com 51% de retenção.

```{r}
churn <- estatistica_qualitativa(churn)
...
descritiva_idade <- estatistica_descritiva(idade_em_anos)
...
descritiva_anos_formacao <- estatistica_descritiva(anos_formacao)
...
descritiva_acs <- estatistica_descritiva(agente_saude)
...
```{r}
descritiva_dentistas <- estatistica_descritiva(dentista)
...
```{r}
descritiva_enfermeiros <- estatistica_descritiva(enfermeiro)
...
```{r}
descritiva_tec_aux_enf <- estatistica_descritiva(tec_aux_enf)
...
```{r}
descritiva_leitos_instalacao_fisica_urgencia <- estatistica_descritiva(leitos_instalacao_fisica_urgencia)
...
```{r}
descritiva_pib_percapita <- estatistica_descritiva(pib_percapita)
...
```{r}
descritiva_tx_pop_ocupada <- estatistica_descritiva(tx_pop_ocupada)
...
```{r}
descritiva_cobertura_plano_saude <- estatistica_descritiva(cobertura_saude)
...
```{r}
descritiva_orcamento_saude <- estatistica_descritiva(orcamento_saude)
...
descritiva_esgoto <- estatistica_descritiva(esgotamento_sanitario)                                                         
data |> 
...
```{r}
descritiva_homicidio <- estatistica_descritiva(taxa_homicidio)
...
descritiva_ideb_inicial <- estatistica_descritiva(ideb_anosiniciais)
...
descritiva_ideb_final <- estatistica_descritiva(ideb_anosfinais)
...
```{r}
descritiva_investimento_infraestrutura <- estatistica_descritiva(investimento_infraestrutura)
...
descritiva_equipamentos_esportivos <- estatistica_descritiva(equipamentos_esportivos)
...
```{r}
descritiva_equipamentos_culturais <- estatistica_descritiva(equpamentos_culturais)
...
```{r}
descritiva_distancia_capital <- estatistica_descritiva(distancia_capital)
...
O código abaixo mostra o percentual de médicos que Permaneceuram nas cidades em que foram designados desagregado por sexo, isto é, a retenção. No entanto, a falta de significância estatística no teste fisher, indica que não há associação entre as variáveis sexo e churn.
...
corr_sexo_churn <- data %>% 
  ...
  mutate(retencao  = round(Permaneceu/(Migrou + Permaneceu)*100,2))
...
fisher.test(corr_sexo_churn[, c("Migrou", "Permaneceu")])
```
...
corr_renovacao_churn <- data |> 
  ...
  count() |> 
  pivot_wider(names_from = churn, values_from = n) |> 
  mutate(retencao = round(Permaneceu / (Permaneceu + Migrou)*100,2)) |> mutate(Prorrogado = ifelse(Prorrogado == "SIM", "Prorrogado", "Não prorrogado"))
...
fisher.test(corr_renovacao_churn[, c("Migrou", "Permaneceu")])
```
...
```{r}
data |> 
  mutate(churn = ifelse(churn == "Migrou", "Migrou", "Permaneceu")) |>
...
data |> 
  ...
  labs(title = "Boxplot: Anos de formação por pessoas migraram e Permaneceuram",
       x = NULL,
       y = "Idade") +
  theme_minimal()
...
data |> 
  ...
  labs(title = "Boxplot: Agentes de Saúde de pessoas por CNES que migraram e Permaneceuram",
       x = NULL,
...
Na mesma lógica anterior, dos agentes de saúde, é realizado com técnico e auxiliares de enfermagem. Os testes indicam não normalidade e diferença estatística não significativa, indicando, que o número de profissionais por estabelecimento de profissionais que migraram e Permaneceuram são iguais.
...
data |> 
  ...
  labs(title = "Boxplot: Tec. e Aux. de Enfermagem por CNES de pessoas que migraram e Permaneceuram",
       x = NULL,
...
Os resultados de enfermeiros são semelhantes dos de técnico e auxiliar de enfermagem, não seguem distribuição normal, e não apresenta diferença estatística significativa no comparativo de estabelecimentos com profissionais que migraram ou Permaneceuram.
...
data |> 
  ...
  labs(title = "Boxplot: Enfermeiros por CNES de pessoas que migraram e Permaneceuram",
       x = NULL,
       y = "Enfermeiros") +
  theme_minimal()
...
Assim como técnicos e auxiliares de enfermagem e enfermeiros, o número de dentistas não segue distribuição normal e não apresenta diferença significativa entre estabelecimentos que tiveram profissionais que Permaneceuram e migraram.
...
data |> 
  ...
  labs(title = "Boxplot: Cirurgiões-dentistas por CNES de pessoas que migraram e Permaneceuram",
       x = NULL,
       y = "Técnicos e Auxiliares de Enfermagem") +
  theme_minimal()
...
corr_regiao_churn <- data |> 
  ...
  count() |> 
  pivot_wider(names_from = churn, values_from = n) |> 
  mutate(retencao = round(Permaneceu / (Migrou + Permaneceu) * 100,0))
...
fisher.test(corr_regiao_churn[, c("Migrou", "Permaneceu")], simulate.p.value = TRUE)
```
...
corr_uf_churn <- data |> 
  ...
  count() |> 
  pivot_wider(names_from = churn, values_from = n) |> 
  mutate(Permaneceu = ifelse(is.na(Permaneceu), 0, Permaneceu)) |> 
  mutate(retencao = round(Permaneceu / (Permaneceu + Migrou) * 100,2))
...
fisher.test(corr_uf_churn[, c("Migrou", "Permaneceu")], simulate.p.value = TRUE)
```
...
fisher.test(corr_porte_churn[, c("Migrou", "Permaneceu")], simulate.p.value = TRUE)
```

Abaixo, o boxplot do PIB per capita desagregado pelo churn. Os testes indicam não normalidade na distribuição de probabilidade e diferença não significativa entre os médicos que Permaneceuram e que migraram. Os resultados mostram que o PIB per capita da cidade não é um determinante para retenção dos médicos, no entanto, deve ser analisado com o ceticismo por não se tratar de uma variável aleatória, dado que os profissionais que selecionam as cidades que desejam ir, tratando dessa forma de um viés.
...
No código abaixo, o boxplot do número de vagas em curso de medicina por município desagregado por churn. Os testes indicam não normalidade da distribuição de probabilidade e diferença estatística significativa entre o número de vagas em municípios em que os médicos Permaneceuram e migraram.
...
Abaixo o boxplot dos gastos públicos com saúde desagregado pelo churn. Os testes indicam não normalidade na distribuição de probabilidade e diferença estatística não significativa entre os municípios que tiveram profissionais que migraram e Permaneceuram.
...
Abaixo o boxplot da cobertura populacional com esgoto sanitário desagregado pelo churn. Os testes indicam não normalidade na distribuição de probabilidade e diferença estatística não significativa entre os municípios que tiveram profissionais que migraram e Permaneceuram.
...
Abaixo o boxplot da taxa de homício dos municípios desagregado pelo churn. Os testes indicam não normalidade na distribuição de probabilidade e diferença estatística não significativa entre os municípios que tiveram profissionais que migraram e Permaneceuram.
...
Abaixo o boxplot da nota do IDEB anos iniciais dos municípios desagregado pelo churn. Os testes indicam não normalidade na distribuição de probabilidade e diferença estatística significativa entre os municípios que tiveram profissionais que migraram e Permaneceuram. Os resultados mostram que a nota ideb dos municípios com maior grau de retenção são ligeiramente maior.
...
Abaixo o boxplot da nota do IDEB anos finais dos municípios desagregado pelo churn. Os testes indicam não normalidade na distribuição de probabilidade e diferença estatística não significativa entre os municípios que tiveram profissionais que migraram e Permaneceuram.
...
Abaixo o boxplot do número de equipamentos esportivos dos municípios desagregado pelo churn. Os testes indicam não normalidade na distribuição de probabilidade e diferença estatística significativa entre os municípios que tiveram profissionais que migraram e Permaneceuram. Os resultados mostram que os municípios com maior grau de retenção tem infraestrutura esportiva maior.
...
Abaixo o boxplot do número de espaços culturais dos municípios desagregado pelo churn. Os testes indicam não normalidade na distribuição de probabilidade e diferença estatística significativa entre os municípios que tiveram profissionais que migraram e Permaneceuram. Os resultados mostram que os municípios com maior grau de retenção tem maior infraestrutura cultural.
...
Abaixo o boxplot da distância da capital em km dos municípios desagregado pelo churn. Os testes indicam não normalidade na distribuição de probabilidade e diferença estatística não significativa entre os municípios que tiveram profissionais que migraram e Permaneceuram.
...
teste <- 
  read_excel("~/GitHub/sa_maismedicos/01_dados/Editais/2019/base_sankeyplot.xlsx") |> 
  mutate(regiao_destino = case_when(
    (uf_destino %in% c("AM", "AP", "TO", 
                       "AC", "RR", "RO", "PA")) ~ "Norte",
    (uf_destino %in% c("BA", "AL", "RN", "SE", 
                       "PI", "PB", "CE", "MA", 
                       "PE")) ~ "Nordeste",
    (uf_destino %in% c("GO", "MT", "MS", 
                       "DF")) ~ "Centro-oeste",
    (uf_destino %in% c("SC", "RS", "PR")) ~ "Sul",
    (uf_destino %in% c("SP", "RJ", "ES", 
                       "MG")) ~ "Sudeste",
    TRUE ~ "Outro")) |> 
  mutate(regiao_final = case_when(
    (uf_final %in% c("AM", "AP", "TO", 
                     "AC", "RR", "RO", "PA")) ~ "Norte",
    (uf_final %in% c("BA", "AL", "RN", "SE", "PI", 
                     "PB", "CE", "MA", "PE")) ~ "Nordeste",
    (uf_final %in% c("GO", "MT", "MS", 
                     "DF")) ~ "Centro-oeste",
