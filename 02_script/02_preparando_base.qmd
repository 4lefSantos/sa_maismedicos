...
codigo_cnes <- temp2 |> 
  filter(HORA_AMB > 30) |> 
  distinct(CPF_PROF, NOMEPROF, CNES)

base_atributos_t <- base_atributos |> 
  left_join(codigo_cnes, 
            by = c("cpf_prof" = "CPF_PROF"))
```
...
leitos_unidades <- 
  ...
  filter(CNES %in% base_atributos$CNES) |> 
  filter(Ano == 2019) |> 
  select(CNES, leitos_instalacao_fisica_urgencia)
  

base_atributos <- leitos_unidades |>
...
base_atributos |> 
  distinct(ibge_aloc, tx_pop_ocupada) |> 
  summarise(média = mean(tx_pop_ocupada))
```
...
base_atributos |> 
  group_by(churn) |> 
  summarise(media = round(mean(tx_pop_ocupada)*100,2),
            mediana = round(median(tx_pop_ocupada)*100,2),
            sd = round(sd(tx_pop_ocupada)*100,2)) |> 
  DT::datatable()
```
...
A partir dos dados da Agência Nacional de Saúde (ANS), agregaremos na nossa base a taxa de cobertura de plano de saúde que é resultado do número de planos de saúde dividido pela população.
...
base_atributos <- base_atributos |> 
  left_join(cobertura_saude, 
            by = c("ibge_aloc" = "cod_ibge"))

base_atributos |> 
  distinct(ibge_aloc, cobertura_saude) |> 
  summarise(média = mean(cobertura_saude))
```

A média de cobertura de saúde dos municípios é de 8,8%, e quando desagregado por churn é maior entre os municípios com indivíduos que permaneceram conforme pode ser visto abaixo.
...
base_atributos |> 
group_by(churn) |> 
summarise(média = round(mean(cobertura_saude),2),
          mediana = round(median(cobertura_saude),2),
          sd = round(sd(cobertura_saude),2))
```

# ALEF, CONFERIR ESTA INFORMAÇÃO ABAIXO EM FONTES EXTERNAS. ESTRANHO... ESPERAVA ALGUM LUGAR DO SUL OU SUDESTE

# DANIEL, REALMENTE OS DADOS ESTÃO DIVERGENDO COM AS FONTES ANS E A POPULAÇÃO (NA CONSULTA) DO IBGE

-   O município com a maior taxa de cobertura de saúde é Paraubebas (PA) com 90%.
...
base_atributos |> 
  ...
  select(cns, uf_destino, 
         ...
         pib_percapita, cobertura_saude, 
         churn) |> 
  DT::datatable()
```

-   O município com a menor taxa de cobertura de saúde é Santa Isabel do Rio Negro (AM) com 0,13%.

```{r}

base_atributos |> 
  filter(cobertura_saude == min(cobertura_saude)) |> 
  select(cns, uf_destino, municipio_destino, populacao, pib_percapita, cobertura_saude, churn) |> 
  DT::datatable()
