...
base_atributos |> 
  group_by(churn) |> 
  summarise(média_anos_formacao = round(mean(anos_formacao),2),
...
```{r}
codigo_cnes <- temp2 |> 
  distinct(CPF_PROF, CNES)

base_atributos <- base_atributos |> 
  left_join(codigo_cnes, by = c("cpf_prof" = "CPF_PROF"))
```
...
base_atributos |> 
  ...
  select(nome, uf_destino, municipio_destino, populacao, porte, churn) |> 
  DT::datatable()
```
...
base_atributos |> 
  ...
  select(nome, uf_destino, municipio_destino, populacao, pib_percapita, churn) |> 
  DT::datatable()
```
...
base_atributos |> 
  ...
  select(nome, uf_destino, municipio_destino, populacao, pib_percapita, tx_pop_ocupada, churn) |> 
  DT::datatable()
```
...
base_atributos |> 
  ...
  select(nome, uf_destino, municipio_destino, populacao, pib_percapita, cobertura_saude, churn) |> 
  DT::datatable()
```
...
base_atributos |> 
  ...
  select(nome, uf_destino, municipio_destino, populacao, pib_percapita, vagas_medicina_milhab, churn) |> 
  DT::datatable()
```
