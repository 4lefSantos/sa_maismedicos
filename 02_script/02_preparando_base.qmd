...
Abaixo visualizaremos a informação desagregada por churn. A prorrogação foi superior entre aqueles que permaneceram.
...
wilcox.test(anos_formacao ~ churn, data = base_atributos)

```

**Contando o número de CPF's na base e verificando se possui mais de um registro associado ao CPF.**

```{r}
count_cpf_atributos <- base_atributos |> 
  group_by(cpf_prof) |> 
  count()
```
...
Mas antes de prosseguirmos, adicionaremos a variável `cnes` que identifica o estabelecimento localizada no dataframe `temp2` , isso será importante para agregar as variáveis dessa seção.

```{r}


temp3 <- base_tratada |> 
  mutate(aps = if_else(TP_UNID %in% c("02", "01", "32", "71", "72", "74"), 
                       "APS", "Outros")) |> 
  mutate(ano = year(COMPETEN)) |> 
  filter(ano %in% c(2019, 2020, 2021, 2022, 2023, 2024)) |> 
  mutate(assumiu_programa = 
           if_else(ano %in% c(2019, 2020, 2021, 2022, 2023, 2024) & 
                   aps == 'APS' & 
                   CODUFMUN == ibge_aloc, 
                   "Foi alocado", 
                   "Não foi alocado"))
...
data_cnes <- temp3 |> 
  filter(assumiu_programa == "Foi alocado") |> 
  distinct(CPF_PROF, CNES, ano) |> 
  filter(CPF_PROF %in% churn$cpf_prof)


teste <- base_atributos |> 
  filter(!cpf_prof %in% data_cnes$CPF_PROF)
```

**A contagem de número de CPF's únicos foi igual que na base de atributos.**

```{r}
count_cpf_cnes <- data_cnes |> 
  group_by(CPF_PROF) |> 
  count()
```
...
leitos_unidades <- 
  ...
  filter(CNES %in% data_cnes$CNES) |> 
  select(CNES, Ano, leitos_instalacao_fisica_urgencia)
  

data_cnes_2 <- data_cnes |>
  left_join(leitos_unidades, 
            by = c("CNES" = "CNES", "ano" = "Ano")) |> 
  group_by(CPF_PROF) |> 
  summarise(m_leitos_instalacao_fisica_urgencia = 
              mean(leitos_instalacao_fisica_urgencia))

base_atributosT <- base_atributos |> 
  left_join(data_cnes_2, by = c("cpf_prof" = "CPF_PROF"))

base_atributos |> 
  distinct(CNES, leitos_instalacao_fisica_urgencia) |> 
  summarise(média = round(mean(leitos_instalacao_fisica_urgencia),2))
```

**Pela contagem abaixo, existem 4 CNES no data_cnes que não está no dataframe leitos_unidades, mas estão associados a 4 cpf distintos, portanto não justifca o tamanho de NA's na variável de leitos.**

```{r}
count_leitos <- data_cnes |> 
  filter(!CNES %in% leitos_unidades$CNES)
```

A média de leitos de instalação física de urgência dos estabelecimentos que receberam médicos do programa é de 5,95 e ao desagregar por churn, não há uma diferença visualmente clara entre daqueles que permaneceram e migraram.
...
a <- base_atributos |> 
  ...
  ylab("Leitos de instalação física de urgência")

b <- base_atributos |>
  distinct(CNES, leitos_instalacao_fisica_urgencia) |> 
  ggplot(aes(x= leitos_instalacao_fisica_urgencia)) + 
  geom_histogram() +
  xlab("Leitos de instalação física de urgência") +
  ylab("Frequência")

a + b
```

Em termos de média, a diferença também parece não ser relevante.

```{r}

base_atributos |> 
  group_by(churn) |> 
  summarise(media = round(mean(leitos_instalacao_fisica_urgencia),2),
            mediana = round(median(leitos_instalacao_fisica_urgencia),2),
            sd = round(sd(leitos_instalacao_fisica_urgencia),2)) |> 
  DT::datatable()
```

-   O estabelecimento de saúde com o maior número de leitos fica em Mazagao no Amapá com 25 leitos.

```{r}
base_atributos |> 
  filter(leitos_instalacao_fisica_urgencia == max(leitos_instalacao_fisica_urgencia)) |> 
  select(nome, uf_destino, municipio_destino, CNES, leitos_instalacao_fisica_urgencia) |> 
  DT::datatable()
```

-   O estabelecimento de saúde com o menor número de leitos fica no Distrito Sanitário Especial Indigena Médio Perus no Amazonas com 0 leitos.

```{r}
base_atributos |> 
  filter(leitos_instalacao_fisica_urgencia == min(leitos_instalacao_fisica_urgencia)) |> 
  select(nome, uf_destino, municipio_destino, CNES, leitos_instalacao_fisica_urgencia) |> 
  DT::datatable()
```
...
base_atributos <- base_atributos |> 
  left_join(data_cnes_3, by = c("cpf_prof" = "CPF_PROF"))
```
...
```{r}
base_atributos |> 
  distinct(CNES, agente_saude) |> 
  summarise(média = mean(agente_saude))
```

A média de agentes comunitários de saúde nos estabelecimentos que receberam médicos do programa é de 8,3. Ao desagregar por churn, observamos uma mediana maior entre os que permaneceram.
...
base_atributos |> 
  group_by(churn) |> 
  summarise(media = round(mean(agente_saude),2),
            mediana = round(median(agente_saude),2),
            sd = round(sd(agente_saude),2)) |> 
  DT::datatable()
```

-   O estabelecimento UNIDADE DE SAUDE DA FAMILIA VILA ROSALI tem maior número de agentes fica em São João de Mereti no Rio de Janeiro.

```{r}
base_atributos |> 
  filter(agente_saude == max(agente_saude)) |> 
  select(nome, uf_destino, municipio_destino, CNES, agente_saude, churn) |> 
  DT::datatable()
```
...
```{r}
base_atributos |> 
  distinct(CNES, tec_aux_enf) |> 
  summarise(média = mean(tec_aux_enf))
```

A média de técnicos e auxiliares de enfermagem nos estabelecimentos é de 2,9. Ao desagregar por churn, pelo boxplot abaixo, visualmente não é possível observar uma diferença clara.
...
Em termos de média a diferença é mais clara, os que permaneceram atuavam em estabelecimentos com uma média de técnicos e auxiliares de enfermagem maior em relação aos que migram.

```{r}

base_atributos |> 
  group_by(churn) |> 
  summarise(media = round(mean(tec_aux_enf),2),
            mediana = round(median(tec_aux_enf),2),
            sd = round(sd(tec_aux_enf),2)) |> 
  DT::datatable()
```

-   O estabelecimento POSTO MEDICO SANITARIO VILA SAO JOAO é o que possui o maior número de técnicos e auxiliares de enfermagem, também localizado em São João de Meriti no Rio de Janeiro que abriga o estabelecimento com o maior número de agentes de saúde.

```{r}

base_atributos |> 
  filter(tec_aux_enf == max(tec_aux_enf)) |> 
  select(nome, uf_destino, municipio_destino, CNES, tec_aux_enf, churn) |> 
  DT::datatable()

```

-   O estabelecimento UNIDADE BASICA DE SAUDE DO DISTRITO DO CAARAPA é o que possui o menor número de técnicos e auxiliares de enfermagem localizado no munícipio Laguna Carapa em Mato Grosso do Sul.

```{r}
base_atributos |> 
  filter(tec_aux_enf == min(tec_aux_enf)) |> 
  select(nome, uf_destino, municipio_destino, CNES, tec_aux_enf, churn) |> 
  DT::datatable()
```
...
base_atributos |> 
  distinct(CNES, enfermeiro) |> 
  summarise(média = mean(enfermeiro))

```

A média de enfermeiros nos estabelecimentos é de 1,7. Ao desagregar por churn não é possível observar uma diferença clara conforme boxplot abaixo.
...
base_atributos |> 
  group_by(churn) |> 
  summarise(media = round(mean(enfermeiro),2),
            mediana = round(median(enfermeiro),2),
            sd = round(sd(enfermeiro),2)) |> 
  DT::datatable()
```

-   O estabelecimento POSTO MEDICO SANITARIO VILA SAO JOAO além de ter o maior número de agentes comunitários de saúde, também possui o maior número de enfermeiro entre os estabelecimentos que receberam médicos do programa.

```{r}
base_atributos |> 
  filter(enfermeiro == max(enfermeiro)) |> 
  select(nome, uf_destino, municipio_destino, CNES, enfermeiro, churn) |> 
  DT::datatable()
```

-   Em relação aos que possui o menor número de enfermeiros, são três estabelecimentos. Entre elas, a UNIDADE BASICA DE SAUDE DO DISTRITO DO CAARAPA de Laguna de Carapa que também possui o menor número de técnico e auxiliares de enfermagem.

```{r}
base_atributos |> 
  filter(enfermeiro == min(enfermeiro)) |> 
  select(nome, uf_destino, municipio_destino, CNES, enfermeiro, churn) |> 
  DT::datatable()
```
...
```{r}
base_atributos |> 
  distinct(CNES, dentista) |> 
  summarise(média = mean(dentista))
```

A média é de 1,1 dentistas nos estabelecimentos que receberam médicos do programa. Ao desagregar por churn no boxplot abaixo não é possível ver diferença em relação a mediana.
...
E em termos de média também não há diferença significativa.

```{r}
base_atributos |> 
  group_by(churn) |> 
  summarise(media = round(mean(dentista),2),
            mediana = round(median(dentista),2),
            sd = round(sd(dentista),2)) |> 
  DT::datatable()
```

-   O estabelecimento UNIDADE DE SAUDE MARIA ANIDEJE possui o maior número de cirurgiões-dentistas localizado em Cambe no Paraná.

```{r}
base_atributos |> 
  filter(dentista == max(dentista)) |> 
  select(nome, uf_destino, municipio_destino, CNES, dentista, churn) |> 
  DT::datatable()
```
...
Observa-se que o Nordeste foi a região que mais recebeu médicos do programa, no apresentou o menor percentual de retenção dos municípios conforme pode ser visto na tabela abaixo.
...
Ceará foi a unidade federativa que mais recebeu médicos do programa, no entanto a retenção ficou em 19º lugar as das melhores retenção conforme pode ser visto na tabela abaixo.
...
Observa-se que a maioria dos médicos foi alocada em cidades de até 50 mil habitantes, visto que PP I e PP II representam aproximadamente 65% dos destinos.
...
A média do PIB per capita dos municípios é de R\$ 18.049,82. Ao desagregar por churn, é possível observar uma mediana maior entre os que permanecem.
...
var_pib <- 
  ...
  mutate(variacao_pib = (ppc_2019 * 100 / ppc_2010) - 100) |> 
...
base_atributos <- 
  ...
  mutate(tx_pop_ocupada_2019 = pop_ocupada/populacao) |> 
...
base_atributos |> 
  ...
  select(cns, uf_destino, municipio_destino, 
         populacao, pib_percapita, tx_pop_ocupada, 
         churn) |> 
  DT::datatable()
```
...
#### 1.4.1.3.1 Variação da população ocupada entre anos

Os dados da população ocupada foram coletados da Estatísticas do Cadastro Central de Empresas (CEMPRE) de 2010 do IBGE por meio do SIDRA, tabela 1685.

E calculamos o percentual da população ocupada com os dados do Censo 2010 por meio do SIDRA, tabela 1378.

```{r}
pop_ocupada_2010 <- read_excel("~/GitHub/sa_maismedicos/01_dados/Editais/2019/pop_ocupada_mun.xlsx") |> 
    select(cod_ibge, `2010`) |> 
  rename(pop_ocupada_2010 = `2010`)

censo_2010 <-  read_excel("~/GitHub/sa_maismedicos/01_dados/Editais/2019/censo_2010.xlsx") |> 
  select(-municipio)

pop_ocupada_2010 <- pop_ocupada_2010 |> 
  left_join(censo_2010, by = c("cod_ibge")) |> 
  mutate(pop_ocupada_2010 = as.numeric(pop_ocupada_2010)) |> 
  mutate(tx_pop_ocupada_2010 = pop_ocupada_2010/populacao_2010) |> 
  select(cod_ibge, populacao_2010, pop_ocupada_2010, tx_pop_ocupada_2010) |> 
  mutate(cod_ibge = substring(cod_ibge, 1, 6))

base_atributos <- base_atributos |> 
  left_join(pop_ocupada_2010, by = c("ibge_aloc" = "cod_ibge")) |> 
  mutate(var_taxa_ocupacao = (tx_pop_ocupada_2019 *100/tx_pop_ocupada_2010) - 100)

base_atributos |> 
  distinct(ibge_aloc, var_taxa_ocupacao) |> 
  summarise(média = mean(var_taxa_ocupacao))
```

A taxa de ocupação apresentou um crescimento relativo de 19,6% entre 2010 e 2019 nos municípios que receberam médicos do programa. Ao desagregar por churn no boxplot abaixo não é possível ver diferença entre os que migraram e permaneceram.

```{r}
a <- base_atributos |> 
  ggplot(aes(x = var_taxa_ocupacao,
             fill = churn)) +
  geom_boxplot() + 
  coord_flip() +
  theme(legend.position = "top") +
...
Em termos de média, os valores também são bem próximos.

```{r}
base_atributos |> 
  group_by(churn) |> 
  summarise(media = round(mean(var_taxa_ocupacao),2),
            mediana = round(median(var_taxa_ocupacao),2),
            sd = round(sd(var_taxa_ocupacao),2)) |> 
  DT::datatable()
```

-   A cidade que apresentou o maior crescimento relativo da taxa de ocupação foi Guaribas do Piauí.

```{r}
base_atributos |> 
  filter(var_taxa_ocupacao == max(var_taxa_ocupacao)) |> 
  select(cns, uf_destino, municipio_destino, 
         populacao,tx_pop_ocupada_2010,tx_pop_ocupada_2019, var_taxa_ocupacao, 
         churn) |> 
  DT::datatable()
```

-   O munípio que apresentou a maior redução relativa da taxa de ocupação foi Cabroso do Pernambuco. (**No entanto o registro de população ocupada de 2010 foi 52.909, valor atípico considerando a série histórica**)

```{r}
base_atributos |> 
  filter(var_taxa_ocupacao == min(var_taxa_ocupacao)) |> 
  select(cns, uf_destino, municipio_destino, 
         populacao,tx_pop_ocupada_2010,tx_pop_ocupada_2019, var_taxa_ocupacao, 
         churn) |> 
  DT::datatable()
...
A média de vagas de medicina por mil habitantes é de 0,3 e visualmente pelo boxplot abaixo não é possível observar diferença desagregada por churn, entende-se apenas que a maioria dos municípios não tem vagas em cursos de medicina.
...
**ALEF, COLOCA MEDIANA E DESVIO-PADRAO NA TABELA ABAIXO. FIZ NAS DEMAIS, COMPLEMENTA ONDE NÃO TIVER**
...
A média de orçamento de saúde dos municípios é de R\$ 697,27 e conforme boxplot abaixo não é possível identificar diferença desagregada por churn.
...
A média da taxa de homicídio dos municípios que receberam médicos do programa é de 26,1 a cada 100 mil habitantes. Na desagregação por churn no boxplot abaixo, a mediana dos municípios de indivíduos que permaneceram é ligeiramente maior.
...
-   O município com maior índice de homicídio é Jussara da Bahia e que recebeu dois médicos do programa.
...
A média dos municípios que receberam médicos do programa é de 5,38. E quando desagregado por churn a mediana dos municípios com médicos que migraram é maior em comparação aos do que permaneceram conforme boxplot abaixo.
...
base_atributos |> 
  group_by(churn) |> 
  summarise(media = round(mean(ideb_anosiniciais),2),
            mediana = round(median(ideb_anosiniciais),2),
            sd = round(sd(ideb_anosiniciais),2)) |> 
  DT::datatable()
```
...
A média da nota ideb dos anos finais é de 4,29, e assim como na nota dos anos inicias, a nota maior nos municípios com médicos que migraram conforme boxplot.
...
base_atributos |> 
  group_by(churn) |> 
  summarise(media = round(mean(ideb_anosfinais),2),
            mediana = round(median(ideb_anosfinais),2),
            sd = round(sd(ideb_anosfinais),2)) |> 
  DT::datatable()
```
...
A média per capita do investimento em infraestrutura dos municípios é de R\$ 3.283,30. Ao desagregar por churn, é possível observar uma mediana maior nas cidades de médicos que migraram.
...
A média de equipamentos culturais por 100 mil habitantes é de 13,5. Ao desagregar por churn, a mediana é maior entre os que migraram conforme boxplot abaixo.
