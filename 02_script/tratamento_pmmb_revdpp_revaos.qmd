...
# Database: Programa Mais Médicos para o Brasil (pmmb)

O objetivo desse trabalho é **elaborar uma base de dados** associado ao Programa Mais Médicos para o Brasil referente ao 18º Ciclo (EDITAL Nº 11, DE 10 DE MAIO DE 2019), o programa é direcionado para atenção básica de saúde.
...
1.1 **Tratamento preliminar**: Com base na identificação dos médicos através de webscraping, é realizada a validação dos profissionais que tomaram posse junto ao levatamento do histórico profissional.
...
Observa-se que a lista de CPF's retornou apenas 2034 observações. Esse número reduzido é devido a médicos duplicados em razão de registros com letras de nomes trocadas. Agora já identificado os CPF, executaremos a consulta abaixo para baixar os dados completos dos profissionais pelos respectivos CPF.
...
2\) e a base do CNES-PF com o histórico profissional dos indivíduos. Os resultados gerados pela consulta.
...
Agora adicionada a coluna de CPF na base cns_inscricao, faremos um left join entre ela e a base CNES_cpf.
...
## 1.2. Atributos a nível indivíduo

A seguir são levantadas as variáveis relacionados aos médicos de forma individualizada, tais como, idade, tempo de formado, média de vínculos, etc.

### 1.2.1 Atuação prévia

A seguir são identificados os médicos que tiveram atuação no município de alocação em anos anteriores ao edital do programa e por quantos anos.
...
Depois de identificado, as informações são agregadas a base original.
...
base_tratada3$anos_atuacao_mesmo_municipio[base_tratada3$atuacao_previa_no_municipio == "Não"] <- 0

```

### 1.2.2 Quantidade média de vínculos enquanto está na mesma cidade

Para isso, vamos verificar, a partir de meados de 2019, quantos vínculos o indivíduos tinha por competência no mesmo município onde ele foi alocado.
...
A seguir, é criado a variável que mede o tempo em meses do tempo de permanência na cidade de designação. Esta é uma das variáveis de desfecho.
...
### 1.2.5 Identificando a UF do médico após 4,5 anos

A seguir é identificada a unidade federativa em que o médico estava atuando após 4,5 anos de início do programa.

```{r}

#Identificando a UF em que o médico está trabalhando na referida competência
base_tratada6.1 <- base_tratada6 %>%
  mutate(uf_exercicio = case_when(
    substr(codufmun, 1, 2) == "11" ~ "RO",
    substr(codufmun, 1, 2) == "12" ~ "AC",
    substr(codufmun, 1, 2) == "13" ~ "AM",
    substr(codufmun, 1, 2) == "14" ~ "RR",
    substr(codufmun, 1, 2) == "15" ~ "PA",
    substr(codufmun, 1, 2) == "16" ~ "AP",
    substr(codufmun, 1, 2) == "17" ~ "TO",
    substr(codufmun, 1, 2) == "21" ~ "MA",
    substr(codufmun, 1, 2) == "22" ~ "PI",
    substr(codufmun, 1, 2) == "23" ~ "CE",
    substr(codufmun, 1, 2) == "24" ~ "RN",
    substr(codufmun, 1, 2) == "25" ~ "PB",
    substr(codufmun, 1, 2) == "26" ~ "PE",
    substr(codufmun, 1, 2) == "27" ~ "AL",
    substr(codufmun, 1, 2) == "28" ~ "SE",
    substr(codufmun, 1, 2) == "29" ~ "BA",
    substr(codufmun, 1, 2) == "31" ~ "MG",
    substr(codufmun, 1, 2) == "32" ~ "ES",
    substr(codufmun, 1, 2) == "33" ~ "RJ",
    substr(codufmun, 1, 2) == "35" ~ "SP",
    substr(codufmun, 1, 2) == "41" ~ "PR",
    substr(codufmun, 1, 2) == "42" ~ "SC",
    substr(codufmun, 1, 2) == "43" ~ "RS",
    substr(codufmun, 1, 2) == "50" ~ "MS",
    substr(codufmun, 1, 2) == "51" ~ "MT",
    substr(codufmun, 1, 2) == "52" ~ "GO",
    substr(codufmun, 1, 2) == "53" ~ "DF",
    TRUE ~ NA_character_ # Caso não corresponda a nenhuma UF
  ))

#Identificando a UF que o médico está trabalhando após 4,5 anos
base_tratada6.2 <- base_tratada6.1 |> 
  mutate(uf_final = ifelse(churn == "permanece", uf_destino, uf_exercicio[competen == as.Date("2023-12-01")])) |> 
  select(-uf_exercicio)

# Exportando a base apenas com variáveis para gerar gráfico de transição (opcional)
#write_xlsx(base_grafico5, "~/GitHub/sa_maismedicos/01_dados/Editais/2019/base_sankeyplot.xlsx" )
```

### 1.2.6 Idade ao entrar no programa

O objetivo desta análise é verificar a idade do indivíduo na primeira competência em que ele começa a trabalhar no programa.
...
O próximo passo é verificar a quanto tempo a pessoa já está formada ao iniciar no programa.
...
Por fim, feitos todos os tratamentos, vamos manter apenas uma observação por indivíduo, referente a primeira competência em que o médico ingressou no programa.

```{r}

base_tratada10 <- 
  base_tratada9 |> 
  filter(alocacao == "Alocado") |> 
  group_by(cpf_prof) |>
  slice_min(order_by = competen, with_ties = FALSE) |>
  ungroup()
```
...
Nessa seção serão agregadas as variáveis a nível municipal a partir de diveras fontes. Essas variáveis são agrupadas de acordo com os seguintes aspectos:
...
A seguir, o PIB per capita dos municípios referentes ao ano de 2019 que marca o inicio do edital 18 do programa.
...
Os dados do quantitativo de população ocupada foram coletados da Estatísticas do Cadastro Central de Empresas (CEMPRE) do IBGE por meio do SIDRA, tabela 1685.
...
A seguir, as variáveis relacionadas a segurança pública do município.
...
A seguir, as variáveis relacionadas ao aspecto educacional do município.
...
A seguir, as variáveis relacionadas a infraestrutura do município.
...
Este dado foi coletado do XXXX, e mostra a distância dos municípios que receberam médicos do programa até a capital da respectiva unidade federativa.
...
base_tratada27$ibge_aloc <- as.character(base_tratada27$ibge_aloc)

base_tratada28 <- base_tratada27 |> 
  left_join(distancia_capital, by = c("ibge_aloc" = "cod_municipio"))

#write_xlsx(base_tratada29, "~/GitHub/sa_maismedicos/01_dados/Editais/2019/base_tratada.xlsx")
```
