...
cns_inscricao <- 
  ...
  filter(!is.na(sexo)) |> 
  mutate(sexo = case_when(sexo == "Male" ~ "Masculino",
...
A seguir, três variáveis foram consideradas como indicadores: taxa de homício, mortes por armas de fogo e taxa de feminicídio, mas os dois últimos foram descartados pelos analistas por entender que a taxa de homicídio já é suficientemente representativa.
...
base_tratada25$ibge_aloc <- as.character(base_tratada25$ibge_aloc)

base_tratada26 <- base_tratada25 |> 
  left_join(distancia_capital, by = c("ibge_aloc" = "cod_municipio"))

#write_xlsx(base_tratada26, "~/GitHub/sa_maismedicos/01_dados/Editais/2019/database.xlsx")
```
