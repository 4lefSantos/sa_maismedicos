...
O primeiro tratamento a ser realizado, é identificar os médicos que iniciaram no programa. Esse mapeamento será realizado através uma variável auxiliar pmmb que identifica se o médico está na cidade no qual foi designado e se está alocado em uma unidade de atenção básica.
...
A seguir será criada uma variável identificando a quantidade de vínculos que os médicos possuem por competência.
...
A seguir, é criado a variável que mede o tempo em meses do tempo de permanência na cidade de designação e em unidade de atenção básica.

```{r}
base_final <- base_final %>%
  group_by(CPF_PROF) %>%
  mutate(
    meses_no_programa = interval(min(COMPETEN[pmmb == "Sim"]), max(COMPETEN[pmmb == "Sim"])) %/% months(1)
  ) %>%
  ungroup()
```
